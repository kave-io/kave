...
func TestInterceptorWarnBehaviorDoesNotBlock(t *testing.T) {
	app, prices := newBudgetStore(t)
	seedBudgetData(t, app, money.MustParseDollars("1"))
	attachPolicy(t, app, "warn", money.MustParseDollars("1"))
	if err := app.InsertBudgetEntry(context.Background(), &runtimemodel.BudgetEntry{
		ID:        "entry-warn",
		ProjectID: "proj-1",
		EnvID:     "env-1",
	...
	ic := New(app, prices)
	action := budgetAction([]byte(`{"model":"gpt-4o","max_tokens":100}`))
	_, err := ic.Before(context.Background(), action)
	if err != nil {
		t.Fatalf("warn behavior should not block: %v", err)
	}
}

func TestInterceptorReserveBehaviorBlocksEstimatedOverrun(t *testing.T) {
	app, prices := newBudgetStore(t)
	seedBudgetData(t, app, money.MustParseDollars("5"))
	attachPolicy(t, app, "reserve", money.MustParseDollars("5"))

	ic := New(app, prices)
	action := budgetAction([]byte(`{"model":"gpt-4o","max_tokens":1000000}`))
	_, err := ic.Before(context.Background(), action)
	if err == nil || !errors.Is(err, ErrBudgetExceeded) {
		t.Fatalf("expected reserve budget block, got %v", err)
	}
	if action.Status != runtime.StatusBlocked {
		t.Fatalf("expected blocked action, got %q", action.Status)
	}
...
func budgetAction(input []byte) *runtime.Action {
	return &runtime.Action{
		Invocation: runtime.Invocation{
			...
			InvocationData: runtime.InvocationData{Input: &input},
		},
	}
}

func attachPolicy(t *testing.T, app store.AppStore, behavior string, cap money.Amount) {
	t.Helper()
	now := time.Now().UnixMilli()
	policyID := "pol-" + behavior
	if err := app.CreatePolicy(context.Background(), &controlmodel.PolicyRecord{
		ID:                policyID,
		ProjectID:         "proj-1",
		EnvID:             "env-1",
		Name:              behavior,
	...
	if err := app.UpdateAgent(context.Background(), "agent-1", &controlmodel.AgentUpdate{PolicyID: &policyID}); err != nil {
