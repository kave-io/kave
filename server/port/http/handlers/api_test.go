...
func TestCreateAgent(t *testing.T) {
	prices, _ := cost.NewService(context.Background(), &mockAppStore{})
	a := New(&mockAppStore{}, &mockSpanStore{}, prices, nil)
...
func TestGetAgent(t *testing.T) {
	...
	prices, _ := cost.NewService(context.Background(), app)
	a := New(app, &mockSpanStore{}, prices, nil)
...
func TestGetRunSpans(t *testing.T) {
	...
	prices, _ := cost.NewService(context.Background(), &mockAppStore{})
	a := New(&mockAppStore{}, spans, prices, nil)
	mux := http.NewServeMux()
	a.RegisterRoutes(mux)
...
func TestPutPricing(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPut, "/api/v1/settings/pricing", bytes.NewReader(body))
	w := httptest.NewRecorder()
	a.putPricing(w, r)
	if w.Code != http.StatusOK {
		t.Fatalf("got %d want 200", w.Code)
	}
	if prices.Current().Version != "custom-1" {
		t.Fatalf("got version %q", prices.Current().Version)
	}
}
