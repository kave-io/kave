...
func (s *SQLiteAppStore) CreateAction(ctx context.Context, a *store.ActionRecord) error {
	metaJSON, _ := json.Marshal(a.Metadata)
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO actions (id, run_id, action_type, connector, method, input, metadata, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, a.ID, a.RunID, a.ActionType, a.Connector, a.Method, string(a.Input), string(metaJSON), a.CreatedAt)
	return err
}

func (s *SQLiteAppStore) GetPriceBook(ctx context.Context) (*store.PriceBook, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *SQLiteAppStore) SavePriceBook(ctx context.Context, book *store.PriceBook) error {
	...
	for i, entry := range book.Entries {
		if _, err := tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, fmt.Sprintf("%s:%d", book.Version, i), book.Version, entry.Provider, entry.Match, entry.Source, entry.InputPerMillion, entry.OutputPerMillion, entry.CacheReadPerMillion, entry.CacheWritePerMillion, i); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *SQLiteAppStore) InsertBudgetEntry(ctx context.Context, entry *store.BudgetEntry) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, entry.ID, entry.WorkspaceID, entry.AgentID, emptyToNil(entry.RunID), ptrEmptyToNil(entry.ActionID), ptrEmptyToNil(entry.SpanID), entry.Connector, entry.Model, entry.InputTokens, entry.OutputTokens, entry.CacheReadTokens, entry.CacheWriteTokens, entry.CostUSD, emptyToNil(entry.PriceVersion), string(snapshotJSON), string(metaJSON), entry.CreatedAt)
	return err
}
...
func (t *txAppStore) CreateAction(ctx context.Context, a *store.ActionRecord) error {
	metaJSON, _ := json.Marshal(a.Metadata)
	_, err := t.tx.ExecContext(ctx, `
		INSERT INTO actions (id, run_id, action_type, connector, method, input, metadata, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
func (t *txAppStore) GetPriceBook(ctx context.Context) (*store.PriceBook, error) {
	return nil, fmt.Errorf("not implemented in transaction")
}

func (t *txAppStore) SavePriceBook(ctx context.Context, book *store.PriceBook) error {
	return fmt.Errorf("not implemented in transaction")
}
...
func ptrEmptyToNil(value *string) any {
	if value == nil || *value == "" {
		return nil
	}
	return *value
